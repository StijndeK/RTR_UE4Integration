...
void JsonIn::readJson(FString filename, AudioSystem& audioSystem)
{
    ...
    // Display the json in a string format inside the output log.
    GLog->Log(JsonString);
...
void JsonIn::initialiseAllValues(AudioSystem& audioSystem, TSharedPtr<FJsonObject>& jsonObject)
{
    ...
    for (int i = 0; i < 5; i++)  audioSystem.getLayerByName(loopLayerNames[i])->onOff = (bool)jsonObject->GetNumberField(loopLayerNames[i].c_str());
}
