...
void Envelopes::setARExp(double attackMs, double releaseMs)
{
    ...
    if (releaseMs != 0) release = pow((amplitudeStartValue / 1.0), 1.0 / (updateRate * releaseMs));
}
void Envelopes::setAExp(double attackMs)
{
    if (attackMs != 0) attack = pow((1.0 / amplitudeStartValue), 1.0 / (updateRate * attackMs + 0.0001));
}
